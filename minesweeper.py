...
class MinesweeperAI():
	...
	def add_knowledge(self, cell, count):
		...
		self.mark_safe(cell)
		neighbors_set, count = self.neighbors(cell, count)
		self.knowledge.append(Sentence(neighbors_set, count))
		changed = True
		while changed:
			changed = False
			...
			self.knowledge[:] = [x for x in self.knowledge if x.cells != set() and x.count != 0]
	...
	def make_random_move(self):
		...
		if moves and not self.knowledge:
			return random.choice(tuple(moves.keys()))
		elif moves:
			for sentence in self.knowledge:
				mine_prob = sentence.count / len(sentence.cells)
				for cell in sentence.cells:
					moves[cell] = mine_prob
			min_val = min(moves.values())
			move = random.choice(list([(key, value) for key, value in moves.items() if value == min_val]))[0]
			return move
		return None